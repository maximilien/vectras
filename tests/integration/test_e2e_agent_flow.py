...
class E2ETestManager:
    ...
    async def step_1_create_failing_tool(self) -> str:
        ...
        create_response = await self.query_agent(
            "testing", "create a simple tool called 'hello' that prints 'Hello World'"
        )
        ...
        assert len(create_response["response"]) > 10, (
            "Agent should provide a meaningful response about tool creation"
        )
    ...
    async def step_2_execute_failing_tool(self, tool_name: str) -> str:
        ...
        assert len(error_response["response"]) > 10, (
            "Log monitor should provide a meaningful response about error patterns"
        )
    ...
    async def step_3_coding_analysis(self) -> str:
        ...
        assert len(response["response"]) > 10, (
            "Code fixer should provide a meaningful response about code analysis"
        )
    ...
    async def step_5_testing_and_linting_verification(self) -> str:
        ...
        # Verify the agent responds meaningfully
        assert len(response["response"]) > 10, (
            "GitHub agent should provide a meaningful response about version control"
        )
    ...
    async def step_6_github_pr_creation(self) -> str:
        ...
        # Verify the agent responds meaningfully
        assert len(response["response"]) > 10, (
            "Agent should provide a meaningful response about coordination"
        )
